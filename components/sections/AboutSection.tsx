...
function ScrollReveal({ children, style, className = "", y = 28, stagger = 0, start = "top 90%" }: { children: React.ReactNode; style?: React.CSSProperties; className?: string; y?: number; stagger?: number; start?: string; }) {
  const ref = useRef<HTMLDivElement>(null);
  const isTouch = useTouchDeviceGuard();
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    const adjustedStart = isTouch ? "top 100%" : start;
    const ctx = gsap.context(() => {
      const targets = stagger > 0 ? el.children : el;
      gsap.fromTo(targets, { opacity: 0, y: y }, { opacity: 1, y: 0, duration: 0.72, stagger: stagger || 0, ease: "power2.out", scrollTrigger: { trigger: el, start: adjustedStart, toggleActions: "play none none reset" } });
    }, ref);
    return () => ctx.revert();
  }, [y, stagger, start, isTouch]);
...
function MLine({ children, delay = 0, style, className = "" }: { children: React.ReactNode; delay?: number; style?: React.CSSProperties; className?: string; }) {
  ...
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    const adjustedStart = isTouch ? "top 100%" : "top 92%";
    const ctx = gsap.context(() => {
      gsap.fromTo(el, { y: "105%", opacity: 0 }, { y: "0%", opacity: 1, duration: 0.84, delay: delay, ease: "power2.out", scrollTrigger: { trigger: el, start: adjustedStart, toggleActions: "play none none reset" } });
    }, ref);
    return () => ctx.revert();
  }, [delay, isTouch]);
  return (
...
function Counter({ target, suffix }: { target: number | string; suffix: string }) {
  ...
  useEffect(() => {
    ...
    if (!isNaN(numVal)) {
      ...
      gsap.fromTo({ v: 0 }, { v: numVal }, { duration: 1.6, ease: "power2.out", onUpdate: function (this: gsap.core.Tween) { if (ref.current) { const currentVal = (this.targets()[0] as { v: number }).v; ref.current.textContent = (isDecimal ? currentVal.toFixed(1) : Math.round(currentVal).toString()) + suffix; } } });
    } else {
...
function useInView(ref: React.RefObject<Element | null>, options: { once?: boolean; margin?: string }) {
  ...
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    const observer = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) { setIsInView(true); if (options.once) observer.unobserve(el); } }, { rootMargin: options.margin });
    observer.observe(el);
...
function TiltCard({ children, style, className = "", maxTilt = 10, maxMove = 8 }: { children: React.ReactNode; style?: React.CSSProperties; className?: string; maxTilt?: number; maxMove?: number; }) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const dx = (x - xc) / xc;
      const dy = (y - yc) / yc;
      gsap.to(card, { rotateX: -dy * maxTilt, rotateY: dx * maxTilt, x: dx * maxMove, y: dy * maxMove, transformPerspective: 1000, duration: 0.25, ease: 'power2.out' });
    };
    const handleMouseLeave = () => { setIsHovered(false); gsap.to(card, { rotateX: 0, rotateY: 0, x: 0, y: 0, duration: 0.5, ease: 'power2.out' }); };
...
function AboutCappedCTAButton({ children, href, onActionClick }: { children: React.ReactNode; href: string; onActionClick?: (e: React.MouseEvent<HTMLAnchorElement>) => void }) {
  return (
    <a href={href} onClick={onActionClick} className="about-cta-beam">
...
function AboutUspStripCard({ item, i, activeIndex, setActiveIndex, mobileActiveIndex, setMobileActiveIndex }: { item: typeof USPS[0]; i: number; activeIndex: number | null; setActiveIndex: (idx: number | null) => void; mobileActiveIndex: number | null; setMobileActiveIndex: (idx: number | null) => void; }) {
  const isTouchDevice = useTouchDeviceGuard();
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.15fr", alignContent: "stretch", overflow: "hidden", borderRadius: "3px", background: "#0a0c0f", width: "100%" }} className="founder-full-card">
        ...
        <div style={{ padding: "48px clamp(24px, 4vw, 48px) 40px clamp(24px, 4vw, 48px)", background: "#0a0c0f", display: "flex", flexDirection: "column", justifyContent: "space-between", zIndex: 1 }} className="founder-card-text-panel">
          <div>
            ...
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "13px", letterSpacing: "0.04em", color: "rgba(255,255,255,0.6)", marginBottom: 20, lineHeight: 1.5 }}>Currently Full-Time Head of Performing Arts at <span style={{color: "#C8960C", fontWeight: 500}}>PREMIER INTERNATIONAL SCHOOL</span></p>
            <p style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(1.1rem, 1.6vw, 1.25rem)", fontStyle: "italic", color: "#C8960C", borderLeft: "2px solid #C8960C", paddingLeft: "14px", marginBottom: "24px", lineHeight: 1.45 }} className="founder-quote-text">"Get the best master you can afford, you’ll find him cheapest in the end."</p>
            <div style={{ width: 30, height: 1, background: "linear-gradient(90deg,#C8960C,transparent)", marginBottom: 24 }} />
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "14px", color: "rgba(245,240,232,0.65)", lineHeight: 1.8, marginBottom: 0, textAlign: "justify" }} className="about-bio-body-text">Academy founder with 10+ years of teaching experience. A Trinity College London certified educator who has guided hundreds of students from age 5 to working professionals to Grade certifications in guitar, keyboard, and violin. His philosophy is simple: teach it right, teach it once.</p>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 20, gap: "10px" }} className="founder-metrics-flex-row">
...
I was reviewing your school details in the academy profile panel and would love to request more structural alignment details regarding your upcoming course batches, musical instruments training loops, and curriculum options. 
...
Could you kindly share the optimal path to connect with the director to discuss enrollment details and clarify current testing schedules? Thank you!`;
    ...
    try {
      await fetch("/api/enroll", {
        ...
        body: JSON.stringify({
          source: "About Us Section Button",
          name: "Anonymous Visitor",
          phone: "Direct Click Interaction",
          email: "Not Provided",
          instrument: "General Explorer",
          message: "Clicked direct WhatsApp shortcut anchor inside Section 001 Heritage block."
        }),
      });
    } catch (error) {
      console.error("Casual interaction tracker pipeline caught exception:", error);
    }
  };
    ...
    <section id="about" className="about-master-global-section" onClick={() => isTouchDevice && setMobileActiveUsp(null)} style={{ position: "relative", background: "#080808", padding: "clamp(60px, 8vw, 120px) 0", overflow: "hidden" }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media (max-width: 768px) { 
          ...
          .founder-card-image-wrapper { aspect-ratio: auto !important; height: 380px !important; }
          .founder-card-text-panel { padding: 36px 24px 32px 24px !important; }
          .founder-quote-text { margin-bottom: 16px !important; font-size: 14px !important; }
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 clamp(20px, 4vw, 40px)", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: "clamp(32px, 5vw, 64px)", alignItems: "stretch", marginBottom: "clamp(54px, 8vw, 96px)" }} className="about-main-grid">
          ...
          <ScrollReveal y={20} style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
            <div>
              ...
              <div style={{ textAlign: "justify" }}>
                ...
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "14px", color: "rgba(245,240,232,0.65)", lineHeight: 1.8, marginBottom: 14 }}>
                  {isTouchDevice && !isParagraphExpanded ? (
                    <>
                      As a Trinity College London Validated Course Provider every lesson is deeply rooted in Staff Notation.{" "}
                      <span 
                        onClick={() => setIsParagraphExpanded(true)} 
                        style={{ color: "#C8960C", cursor: "pointer", fontWeight: 600, textDecoration: "underline", marginLeft: "4px" }}
                      >
                        Read More
                      </span>
                    </>
                ...
                {(!isTouchDevice || isParagraphExpanded) && (
                  <motion.p 
                    initial={isTouchDevice ? { opacity: 0, y: 6 } : false}
                    animate={isTouchDevice ? { opacity: 1, y: 0 } : false}
                    style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "14px", color: "rgba(245,240,232,0.65)", lineHeight: 1.8, marginBottom: 14 }}
                  >
                    ...
                    {isTouchDevice && isParagraphExpanded && (
                      <span 
                        onClick={() => setIsParagraphExpanded(false)} 
          ...
          <div className="about-usp-accordion-stack-container" style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", gap: "12px", width: "100%", height: isTouchDevice ? "480px" : "100%" }}>
            {USPS.map((item, i) => (
              <AboutUspStripCard key={i} item={item} i={i} activeIndex={activeUspIndex} setActiveIndex={setActiveUspIndex} mobileActiveIndex={mobileActiveUsp} setMobileActiveIndex={setMobileActiveUsp} />
            ))}
          </div>
        </div>
        ...
        <ScrollReveal y={32}>
          <div onMouseEnter={() => !isTouchDevice && setLocHover(true)} onMouseLeave={() => !isTouchDevice && setLocHover(false)} className={`contact-premium-card ${locHover && !isTouchDevice ? "is-hovering" : ""}`} style={{ padding: "1.5px", background: "rgba(255, 255, 255, 0.06)", borderRadius: "4px" }} >
            ...
            <div style={{ position: "relative", zIndex: 2, background: "#0a0c0f", padding: "clamp(24px, 3.5vw, 36px) clamp(24px, 4vw, 40px)", borderRadius: "3px", display: "grid", gridTemplateColumns: "auto 1fr auto", gap: "24px", alignItems: "center", width: "100%" }} className="location-strip-grid">
              ...
              <div className="hero-location-inner-stack" style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
